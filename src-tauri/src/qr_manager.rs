    ...
    fn choose_preferred_host() -> String {
        ...
        if let Ok(map) = list_afinet_netifas() {
            ...
            for (name, ip) in map.into_iter() {
                ...
                // ループバック/リンクローカルは除外
                if v4.is_loopback() || v4.octets()[0] == 169 { continue; }

                // 優先度（小さいほど優先）: Wi-Fi(en*) < 有線(eth*) < 無線(wl*) < それ以外
                let mut score = 100;
                let lower = name.to_lowercase();
                if lower.starts_with("en") { score = 10; }
                else if lower.starts_with("eth") { score = 20; }
                else if lower.starts_with("wl") { score = 30; }

                // 明示的に除外したい仮想/特殊IFはスコアを下げない（実質候補外）
                if lower.starts_with("awdl") || lower.starts_with("llw") || lower.starts_with("utun") || lower.contains("bridge") {
                    continue;
                }
        ...
        local_ip().map(|ip| ip.to_string()).unwrap_or_else(|_| "localhost".to_string())
    }
